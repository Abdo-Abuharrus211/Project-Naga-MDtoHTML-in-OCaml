(*
	...
	This is the "Naga" MD project module.
	Naga MD, is a simple Markdown to HTML converter that utilizes OCaml's pattern matching and functional 
...
(*	Function that identifies and parses a heading, and returns a variant `Heading` type
	that contains heading size and text.
...
(* Function that parses bulleted lists (both the `-` and `*` variety) and returns a 'BulletList` variant.*)
...
(* Function to identify numbered lists *)
...
(* Function to parse URL elements, extracting the text and the URL *)
let parse_url line =
  try
...
(* `Generic parser to process a line and classify it .
	Parses a single line into an md_element *)
